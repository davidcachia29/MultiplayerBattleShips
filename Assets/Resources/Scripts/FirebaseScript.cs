...
public class FirebaseScript : MonoBehaviour
{
    ...
    string email = "davidcachia29@gmail.com";
    string password = "TestTestTest";

    

    public IEnumerator updateData(string childlabel, string newdata)
    {

        bool dataupdated = false;
        //find the child of player4 that corresponds to playername and set the value to whatever is inside newdata
        reference.Child("Admin").Child(childlabel).SetValueAsync(newdata).ContinueWithOnMainThread(
            updateValueTask =>
            {
                if (updateValueTask.IsCompleted)
                {
                ...
                if (updateValueTask.IsFaulted)
                {
                    Debug.Log("Sorry, data was not updated" + updateValueTask.Exception);
                }
            });        
    ...
    public IEnumerator addDataClass(string datatoinsert,gameManager g,Text Id)
    {
        //create a unique ID
    ...
    public IEnumerator updateDataClass(string childlabel, string newdata)
    {
    ...
    public IEnumerator downloadAndSaveImage()
    {
        ...
        StorageReference storage_ref = storage.GetReferenceFromUrl("gs://syno-b52f4.appspot.com/Circle.png");
   ...
   public IEnumerator clearFirebase()
    ...
    public IEnumerator getNumberOfRecordsAfterStarted()
    {
        Task numberofrecordstask = reference.GetValueAsync().ContinueWithOnMainThread(
             getValueTask =>
             {
   ...
   public  IEnumerator getAllDataFromFirebase()
    {
        ...
        //the data has been saved to snapshot here
        yield return StartCoroutine(displayIndividualData());
    }

    IEnumerator displayIndividualData()
    {
        

        foreach (var element in myDataDictionary)
        {
            showData(element.Value).ToString();           
        }
        yield return null;
        
    }
    string showData(object dictionary)
    {
        
        string output = "";
        Dictionary<string, object> leafdata = (Dictionary<string, object>)dictionary;
       

        foreach (var element in leafdata)
        {
            Data.Add(element.Key.ToString());
    ...
    {
        if (!signedin) { 
            FirebaseApp.DefaultInstance.SetEditorDatabaseUrl("https://dissertation-79043-default-rtdb.firebaseio.com/");
        ...
        StartCoroutine(getAllDataFromFirebase());
    }
    ...
    private void Update()
    {
        ...
        //{
        //    string filename = Application.persistentDataPath + "/Circle.png";
        //    warship = LoadSprite(filename);
        //    Debug.Log(warship);

        //    BackGround = GameObject.Find("backgroundImage");


        //    var item = GameObject.Find("backgroundImage");
        //    if (item == null)
        //    {
        //        Debug.Log("Item Not Found");
        //    }

        //    //BackGround.GetComponent<Image>().sprite = warship;
        ...
        //if (SceneManager.GetActiveScene().name == "battleships")
        //{
        //    string filename = Application.persistentDataPath + "/Circle.png";
        //    warship = LoadSprite(filename);
        //    Debug.Log(warship);
        ...
        //    BackGround.GetComponent<SpriteRenderer>().sprite = warship;
        //}
